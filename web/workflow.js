...
async function startPlannerResearch(){ const a=plannerChat.ready,budget=Number(String(a.budget).replace(/[^0-9]/g,''))||0; Object.assign(state,{departure:a.home_region,destination:a.destination,nights:a.nights,themes:a.themes.split(/[、,・]/).filter(Boolean),preference:a.spot_preference,budgetMin:budget,budgetMax:budget,accommodation:a.lodging_type,transport:a.transport.split(/[、,・]/).filter(Boolean),requests:a.special_requests}); workflow.notice='インターネット上の無料公開情報を検索しています。設定済みの場合はGoogle Maps候補も追加します。'; workflow.previewEvidence=[]; persistWorkspace(); try { const [research,connected]=await Promise.all([requestJson('/api/free-research',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({destination:a.destination})}),requestJson('/api/connected-research',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({destination:a.destination})})]); const candidates=[...(research.evidence||[]),...(connected.evidence||[])]; workflow.previewEvidence=candidates.map(item=>({title:item.title,source_type:item.source_type,url:item.url,retrieved_at:item.retrieved_at})); if(candidates.length){const preview=await requestJson('/api/workspace/draft-preview',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({requirements:{destination:a.destination,nights:a.nights},evidence:candidates})});workflow.draft=preview.draft;const googleNote=connected.state==='available'?' Google Maps候補 '+connected.evidence.length+'件も追加しました。':' Google Mapsは未設定のため、無料公開情報だけを使いました。';workflow.notice='インターネット検索で取得した'+candidates.length+'件の未検証候補から、保存前の旅程下書きを表示しています。'+googleNote;}else{workflow.notice='インターネット上の公開情報から候補を取得できませんでした。調査runは作成済みです。';}}catch(error){workflow.notice='インターネット検索による下書き候補を取得できませんでした: '+error.message;} persistWorkspace(); plannerChat.generating=false; persistPlannerChat(); await refreshAutoWorkerStatus(); render('assistant'); }
...
runResearch = async function () {
  ...
  try {
    const [research, connected] = await Promise.all([
      ...
      requestJson('/api/connected-research', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ destination: state.destination }) })
    ]);
    evidence = [...(research.evidence || []), ...(connected.evidence || [])];
    const targets = ['nominatim', 'wikimedia', 'open_meteo']; if (connected.state === 'available') targets.push('google_maps');
    const created = await requestJson('/api/workspace/runs', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ requirements: requirements(), source_targets: targets }) });
