...
function assistantDraftCard() {
  ...
  return '<section id="assistant-draft" class="card assistant-draft"><div class="auto-worker-head"><div><h2>自動生成した旅程下書き</h2><p>' + esc(draft.notice) + '</p></div><span class="chip research">' + esc(draft.status) + '</span></div><h3>' + esc(draft.title) + '</h3><ol>' + draft.days.map(day => '<li><b>' + esc(day.day) + '日目・' + esc(day.label) + '</b>　' + esc(day.focus) + '<br><small>根拠: ' + esc(day.source) + ' ／ 時刻: ' + esc(day.time) + ' ／ 移動: ' + esc(day.transport) + ' ／ 費用: ' + esc(day.cost) + '</small></li>').join('') + '</ol><div class="save-block"><b>保存・公開はブロック中</b><p>' + esc(draft.save_blocked_reason) + '</p></div><button type="button" class="secondary" data-go="research">根拠候補を確認する →</button></section>';
}
...
async function startPlannerResearch(){ const a=plannerChat.ready,budget=Number(String(a.budget).replace(/[^0-9]/g,''))||0; Object.assign(state,{departure:a.home_region,destination:a.destination,nights:a.nights,themes:a.themes.split(/[、,・]/).filter(Boolean),preference:a.spot_preference,budgetMin:budget,budgetMax:budget,accommodation:a.lodging_type,transport:a.transport.split(/[、,・]/).filter(Boolean),requests:a.special_requests}); workflow.notice='調査候補を取得しています。'; persistWorkspace(); try { const research=await requestJson('/api/free-research',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({destination:a.destination})}); if(research.evidence&&research.evidence.length){const preview=await requestJson('/api/workspace/draft-preview',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({requirements:{destination:a.destination,nights:a.nights},evidence:research.evidence})});workflow.draft=preview.draft;workflow.notice=research.evidence.length+'件の未検証候補から、保存前の旅程下書きを表示しています。';}else{workflow.notice='公開情報から候補を取得できませんでした。調査runは作成済みです。';}}catch(error){workflow.notice='下書き候補を取得できませんでした: '+error.message;} persistWorkspace(); plannerChat.generating=false; persistPlannerChat(); await refreshAutoWorkerStatus(); render('assistant'); }
